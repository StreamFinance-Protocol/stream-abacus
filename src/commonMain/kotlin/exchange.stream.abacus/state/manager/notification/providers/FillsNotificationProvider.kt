...
class FillsNotificationProvider(
    ...
    override fun buildNotifications(
        ...
        if (subaccountFills != null) {
            // Cache the orders
            val orders = exchange.stream.abacus.utils.mutableMapOf<String, SubaccountOrder>()
            for (order in subaccount.orders ?: iListOf()) {
                orders[order.id] = order
            }
            // Cache the fills
            val fills =
                exchange.stream.abacus.utils.mutableMapOf<String, IMutableList<SubaccountFill>>()
