...
import exchange.stream.abacus.state.manager.CctpChainTokenInfo
import exchange.stream.abacus.state.manager.CctpConfig
import exchange.stream.abacus.state.manager.CctpWithdrawState
import exchange.stream.abacus.state.manager.ExchangeConfig
import exchange.stream.abacus.state.manager.ExchangeInfo
import exchange.stream.abacus.state.manager.HumanReadableDepositPayload
import exchange.stream.abacus.state.manager.HumanReadableFaucetPayload
import exchange.stream.abacus.state.manager.HumanReadableSubaccountTransferPayload
import exchange.stream.abacus.state.manager.HumanReadableTransferPayload
import exchange.stream.abacus.state.manager.HumanReadableWithdrawPayload
import exchange.stream.abacus.state.manager.Platform
import exchange.stream.abacus.state.manager.RpcConfigs
import exchange.stream.abacus.state.manager.StatsigConfig
import exchange.stream.abacus.state.manager.SystemUtils
...
import exchange.stream.abacus.state.model.routerChains
import exchange.stream.abacus.state.model.routerStatus
import exchange.stream.abacus.state.model.routerTokens
import exchange.stream.abacus.state.model.routerTrack
import exchange.stream.abacus.state.model.squidRoute
import exchange.stream.abacus.state.model.squidRouteV2
import exchange.stream.abacus.state.model.squidV2SdkInfo
...
import exchange.stream.abacus.utils.toNeutronAddress
import exchange.stream.abacus.utils.toNobleAddress
import exchange.stream.abacus.utils.toOsmosisAddress
...
//      We can consider deduping this at a later point.
        ...
        val fromAmountString = helper.parser.asString(fromAmount) ?: return
        val url = helper.configs.skipV2MsgsDirect()
        val toChain = helper.environment.streamChainId ?: return
    ...
    private fun retrieveSkipWithdrawalRouteExchange(
        ...
        if (fromAmount <= 0) return
        val fromChain = helper.environment.streamChainId ?: return
        val nativeChainUSDCDenom = helper.environment.tokens["usdc"]?.denom ?: return
        val fromAmountString = helper.parser.asString(fromAmount) ?: return
        val url = helper.configs.skipV2MsgsDirect()
        val fromAddress = accountAddress
